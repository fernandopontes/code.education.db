...
?>
<div class="col-md-12">
    <h1 class="blog-title"><strong>Produtos</strong></h1>
    <br><br>
    <a href="admin?pagadmin=produtos&acao=cadastrar" class="btn btn-success">Cadastrar novo</a>
    <br><br>
    <?php
    if(!isset($_GET['acao']))
    {
        ...
        if(count($resultado) > 0)
        {
            ...
            {
                printf('<tr><td>%s</td><td><a href="admin?pagadmin=produtos&acao=editar&id=%d" class="btn btn-primary">Editar</a></td><td><a href="admin?pagadmin=produtos&acao2=excluir&id=%d" class="btn btn-danger">Excluir</a></td></tr>', $resultado[$i]['titulo'], $resultado[$i]['id'], $resultado[$i]['id']);
            }
    ...
    {
    ?>
    <form action="admin?pagadmin=produtos" method="post">
        <?php
        $consulta = $conexao->prepare('SELECT id, titulo, texto FROM site_produtos WHERE id=:id');
        $id = $_GET['id'];
        ...
        <input type="hidden" name="pagina_editada" value="produtos">
        <br>
    ...
    <form action="admin?pagadmin=produtos" method="post">
        ...
        <input type="hidden" name="pagina_cadastrar" value="produtos">
        <br>
        <p><input type="submit" name="submit" value="Salvar" class="btn btn-primary"></p>
    </form>
    <?php
